<?php
	...
	if(isset($_GET['id_livr2']))
    {
        $req=$bdd->prepare('UPDATE livraison SET status=:status WHERE codeli=:codeli') or die(print_r($bdd->errorInfo()));
        $req->execute(array(
            ':codeli'=>$_GET['id_livr2'],
            ':status'=>'Annuler'
        ));
        echo '<script>alert("Votre modification du status a ete prise en compte")</script>';
    }

    if(isset($_GET['id_livr3']))
    {
        $req=$bdd->prepare('UPDATE livraison SET status=:status, dateLivraison=NOW() WHERE codeli=:codeli') or die(print_r($bdd->errorInfo()));
        $req->execute(array(
            ':codeli'=>$_GET['id_livr3'],
            ':status'=>'En cours'
        ));
        echo '<script>alert("Votre modification du status a ete prise en compte")</script>';
    }
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a><?php echo '<a href="liste-livraison.php?id_livr3='.$donnees['codeli'].'">';?><button class="btn btn-primary btn-xs">En cours</button></a>
                                        <a><?php echo '<a href="liste-livraison.php?id_livr2='.$donnees['codeli'].'">';?><button class="btn btn-primary btn-xs">Annuler</button></a>
                                    </td>
                                </tr>
                                <?php
                                }
